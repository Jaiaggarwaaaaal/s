...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['ENIAC_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ENIAC_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Eniac API.
 */
export class Eniac {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['ENIAC_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('ENIAC_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.EniacError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Eniac client with an apiKey option, like new Eniac({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Eniac.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ENIAC_LOG'), "process.env['ENIAC_LOG']", this) ??
      defaultLogLevel;
  ...
  static EniacError = Errors.EniacError;
...
Eniac.PetResource = PetResource;
Eniac.Store = Store;
Eniac.UserResource = UserResource;
export declare namespace Eniac {
